...
interface EditServiceModalProps {
  service: Service;
  onClose: () => void;
  onUpdate: (id: number, data: { name: string; startingPrice: number }) => Promise<void>;
  onSuccess?: () => void;
  onError?: (error: string) => void;
}

export default function EditServiceModal({ service, onClose, onUpdate, onSuccess, onError }: EditServiceModalProps) {
  const [name, setName] = useState(service.name);
  const [startingPrice, setStartingPrice] = useState(service.starting_price.toString());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await onUpdate(service.id, { name: name.trim(), startingPrice: priceNum });
      onSuccess?.();
      onClose();
    } catch (err) {
